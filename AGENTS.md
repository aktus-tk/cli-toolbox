...
7. 未確認のスクリプトを `curl | bash` で直接実行しない。
   ただし、Claude Code など、公式提供元が標準のインストール方法として案内しているインストーラーは使用を許可する。
   取得先は必ず公式ドメインであることを確認する。
   公式性を確認できない場合は実行せず、可能であればスクリプトを保存して内容を確認する。
8. 公式配布元だけを使用する
